...
【絶対条件】questionsの配列に最低10問以上を含めてください。15問程度が理想です。5問以下の場合は出力として不合格です。
全てのカテゴリを使う必要はありませんが、重要なカテゴリ（BIZ, FIN, ORG, RSK, GRW）を中心に幅広くカバーしてください。
質問は優先度A→B→Cの順にソートして出力してください。
{category_list}

■ セクション3: 主要論点・リスク
資料を横断的に分析し、ディールの成否に関わる主要な論点・リスクを挙げてください。
【絶対条件】key_issuesの配列に最低5点以上を含めてください。8点程度が理想です。3点以下は不合格です。
...
- 業績推移（P/L）: 売上高・営業利益・経常利益・当期純利益・EBITDAを年度別に。
  【重要】複数期間のCSVがある場合は**必ず全期間分**をpl_trendsに出力。期間A/B/Cの表記はIMと照合して正しい会計年度名に変換。
  さらに、IMに追加の年度データ（CSVに含まれない過去の年度）がある場合、そのデータもpl_trendsに追加してください。
  目標: CSVとIMを合わせて3ヵ年分のP/Lを出力。
- BS推移: 総資産・負債合計・純資産・現預金・有利子負債を年度別に。複数期間分を必ず出力。IMに追加年度があればそれも含める。
...
※ 金額について文章で言及する場合は「約X億円」「約X万円」の形式で正確に換算してください（例: 656,872,759円 → 約6.6億円）。

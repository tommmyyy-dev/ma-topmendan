...
3. のれん償却の影響: 上場企業として業績インパクトを必ず試算
4. PMIの実現可能性: iHack統合経験を踏まえた評価
...
{doc_text}
{fin_block}
以下の構成でMarkdownレポートを出力してください。簡潔・具体的に。

---

## 1. ディール概要
対象企業の概要（事業内容、規模、ポジション）を400〜600字で。

## 2. 対象企業の魅力（トップ3）
買い手にとっての魅力を **3項目**。各100〜150字で具体的に。箇条書き。

## 3. 主要懸念事項（トップ3）
リスク・懸念を **3項目**。各100〜150字で具体的に。箇条書き。

## 4. セラーの譲渡動機（仮説）
最も有力な仮説を **2つ**。各100字程度で。

## 5. シナジー仮説（トップ2）
買い手プロファイルを踏まえ、**最も実現性が高い2つ** に絞る。中期目標（売上50億・営利8億）への貢献度を定量的に評価。ジーニーグループに関する言及は不要。

## 6. 財務分析

### 6-1. 業績トレンド
売上高・営業利益・経常利益・EBITDAの推移をMarkdownテーブルで。簡潔なトレンドコメント付き。

### 6-2. EBITDA
IM・財務資料に記載されているEBITDAをそのまま記載。テーブル形式。余計な感応度分析は不要。

### 6-3. バリュエーション試算
**算出方法: EBITDA × マルチプル + 純資産 = 評価額**
- 3x / 4x / 5x の3パターンでテーブル形式で試算。
- 純資産額を明記し、最終的な評価額レンジを提示。

### 6-4. のれん償却シミュレーション
想定取得価額でののれん額と、5年・10年・20年償却時の年間償却額を試算。買い手PLへのインパクトを評価。テーブル形式。

### 6-5. 財務リスク（トップ3）
財務面のリスクを **3項目**。各100〜150字。箇条書き。

## 7. トップ面談で確認すべき質問（10問）
優先度の高い順に10問。各質問に質問意図を1行で付記。
...
他のアナリストの分析結果を客観的かつ厳しくレビューします。

【判定ルール】
- GO / CONDITIONAL-GO / NO-GO を案件の実態に基づいて公正に判定する
- デフォルトNO-GOではない。良い案件にはGOを出す勇気を持つ
- ただし忖度は不要。リスクは正直に指摘する
- 数値の前提に疑問を呈する
- 最悪のシナリオも想定する
日本語で出力。"""
